class UserOverlay extends React.Component {
            ...
            return (
                <li key={user.id}>
                    ...
                    <a href={user.urlPath} className="overlay-user-info">
                        <strong>{<UserNameWithStatus
                            user={{
                                username: user.username,
                                value: user.isVerifiedMember
                            }}/>}</strong>
                        <p>{user.description}</p>
                    </a>
                    <small dangerouslySetInnerHTML={this.renderVerifiedMemberBadge(user)}/>
    ...
    renderVerifiedMemberBadge(user) {
      if (user.verifiedMember) {
          return <img src='verified.svg' />
      }
    }
    ...
    renderPrevButton() {
        if (this.state.currentPage > 1) {
            return (
                <a className="button" onClick={this.handlePrevClick}>{I18n.t('heading.previous')}</a>
            );
        }
    }
    ...
    renderNextButton() {
        if (this.state.nextPage !== null) {
            return (
                <a className="button" onClick={this.handleNextClick}>{I18n.t('heading.next')}</a>
            );
        }
    }
